...
namespace ArkanoidGame
{
	void GameStatePlayingData::Init()
	{
		...
		assert(gameOverSoundBuffer.loadFromFile(SETTINGS.SOUNDS_PATH + "Bonuse.wav"));

		//factoriesInit
		factories.emplace(BlockType::Simple, std::make_unique<SimpleBlockFactory>());
		factories.emplace(BlockType::TwoHit, std::make_unique<TwoHitBlockFactory>());
	...
	void GameStatePlayingData::Update(float timeDelta)
	{
				...
				[ball, &hasBrokeOneBlock, &needInverseDirX, &needInverseDirY, this](auto block) 
		{
			if ((!hasBrokeOneBlock) && block->CheckCollision(ball)) 
			{
				...
				std::unique_ptr<ScoringStrategy> scoringStrategy = std::make_unique<BasicBlockScoring>();
		     	int point = scoringStrategy->calculateScore(block);
				currentScore += point;
		...
		scoreText.setString("Score: " + std::to_string(breackableBlocksCount));
	}
