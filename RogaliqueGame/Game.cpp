...
Game::Game() {
    ...
    recordsTable = {
        {"John", SETTINGS.MAX_SCORE / 2},
        {"Jane", SETTINGS.MAX_SCORE / 3},
        {"Alice", SETTINGS.MAX_SCORE / 4},
        {"Bob", SETTINGS.MAX_SCORE / 5},
        {"Clementine", SETTINGS.MAX_SCORE / 5},
    };
...
void Game::HandleWindowEvents(sf::RenderWindow& window) {
    ...
    while (window.pollEvent(event)) {
        ...
        if (stateStack.size() > 0) {
            stateStack.back().HandelWindowEvent(event);
        }
    }
}
...
void Game::Draw(sf::RenderWindow& window) {
    if (stateStack.size() > 0) {
        ...
        for (auto it = visibleGameStates.rbegin();
             it != visibleGameStates.rend(); ++it) {
            (*it)->Draw(window);
        }
    }
}
...
bool Game::IsEnableOptions(GameOptions option) const {
    const bool isEnable = ((std::uint8_t)options & (std::uint8_t)option) !=
                          (std::uint8_t)GameOptions::Empty;
    return isEnable;
}
...
void Game::ExitGame() { SwitchStateTo(GameStateType::MainMenu); }
void Game::ShowRecords() { PushState(GameStateType::Records, true); }

void Game::QuitGame() { SwitchStateTo(GameStateType::None); }

void Game::StartGame() { SwitchStateTo(GameStateType::Playing); }

void Game::PauseGame() { PushState(GameStateType::ExitDialog, false); }

void Game::WinGame() { PushState(GameStateType::GameWin, false); }

void Game::LooseGame() { PushState(GameStateType::GameOver, false); }
