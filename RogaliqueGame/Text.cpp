...
void DrawTextList(sf::RenderWindow& window, const std::vector<sf::Text*>& items,
    ...
    for (auto it = items.begin(); it != items.end(); ++it) {
        ...
        if (orientation == Orientation::Horizontal) {
            itemOrigin.y = alignment == Alignment::Min      ? 0.f
                           : alignment == Alignment::Middle ? 0.5f
                                                            : 1.f;
            itemOrigin.x = 0.f;
            currentPos.y = totalRect.top + itemOrigin.y * totalRect.height;
        } else {
            itemOrigin.y = 0.f;
            itemOrigin.x = alignment == Alignment::Min      ? 0.f
                           : alignment == Alignment::Middle ? 0.5f
                                                            : 1.f;
